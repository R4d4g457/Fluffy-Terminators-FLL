# SPIKE Prime (LEGO lessons API)
# - Ports: from hub import port
# - Motion sensor: motion_sensor.tilt_angles(), motion_sensor.reset_yaw(0)
# - Motors: motor.run(), motor.stop(), motor.relative_position(), motor.reset_relative_position(port, 0)
# - Motor pair: motor_pair.pair(), motor_pair.move(..., velocity=...), motor_pair.stop()
# - Color sensor: color_sensor.reflection(port.X)
# - Force sensor: force_sensor.pressed(port.X)
...
# ---------------- control shims (local polling helpers) ----------------

def wait_for_seconds(sec):
    try:
        utime.sleep(max(0.0, float(sec)))
    except Exception:
        pass
...
def wait_until(pred, timeout=None, poll_ms=10):
    ...
    while True:
        ...
        if timeout is not None:
            if utime.ticks_diff(utime.ticks_ms(), start_ms) >= int(float(timeout) * 1000.0):
                return False
...
def stop_motor(p):
    """Stop motor (no mode arg in this API)."""
    motor.stop(p)
...
def line_follow_speed_gain_target_lineside_port(speed, gain, target, lineside, color_port):
    """
